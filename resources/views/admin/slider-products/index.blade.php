    ...
    <div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            <div class="d-flex align-items-center flex-wrap mr-2">
                <h5 class="text-dark font-weight-bold my-1 mr-5">{{ $title }} Slider</h5>
                <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.home') }}" class="text-muted">Dashboard</a>
                    </li>
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.slider-products.index', ['type' => 'fall_winter']) }}" class="text-muted">Home Sliders</a>
                    </li>
                    <li class="breadcrumb-item">
            ...
            <div class="d-flex align-items-center">
                <div class="btn-group mr-2">
                    <a href="{{ route('admin.slider-products.index', ['type' => 'fall_winter']) }}" 
                       class="btn btn-sm {{ $type === 'fall_winter' ? 'btn-primary' : 'btn-light' }}">
                        FALL / WINTER
                    </a>
                    <a href="{{ route('admin.slider-products.index', ['type' => 'spring_summer']) }}" 
                       class="btn btn-sm {{ $type === 'spring_summer' ? 'btn-primary' : 'btn-light' }}">
                        SPRING / SUMMER
                    </a>
                </div>
            </div>
            ...
            <div class="card card-custom">
                <div class="card-header border-0 pt-6">
                    <div class="card-title w-100">
                        <h3 class="card-label">{{ $title }} Slider Products</h3>
                    </div>
                </div>
                <div class="card-body pt-0">
                    ...
                    <div class="product-search-wrapper">
                        ...
                        <span class="form-text text-muted">Search by SKU or name and select the product to add.</span>
                    </div>
    ...
    <script>
        ...
        var KTDatatablesServerSide = function () {
            ...
            var initDatatable = function () {
                table = $('#kt_datatable').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('admin.slider-products.data') }}",
                        type: 'GET',
                        data: {
                            type: '{{ $type }}'
                        },
        ...
        jQuery(document).ready(function () {
            ...
            $('#product_search').select2({
                ...
                ajax: {
                    url: "{{ route('admin.slider-products.search') }}",
                    type: 'GET',
                    dataType: 'json',
                    delay: 300,
            ...
            $('#product_search').on('select2:select', function (e) {
                ...
                addProductToSlider(data.id, '{{ $type }}');
            });
        });
        ...
        function addProductToSlider(productId, sliderType) {
            Swal.fire({
                title: 'Adding...',
                text: 'Please wait...',
            ...
            $.ajax({
                url: "{{ route('admin.slider-products.store') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
        ...
        function deleteSlider(sliderId) {
            Swal.fire({
                ...
                text: 'Are you sure you want to remove this product from the slider?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = "{{ route('admin.slider-products.destroy', ':id') }}".replace(':id', sliderId);
